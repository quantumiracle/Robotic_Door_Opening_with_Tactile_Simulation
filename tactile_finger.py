...
class Model(Task):
    ...
    def place_objects(self, distance):
        for i in range(len(self.objects)):
            self.objects[i].set("pos", array_to_string(np.array([0, 0.0292+0.0001*distance, 0.008])))  # 0.0128, 0.0256, 0.035 (0.006)
...
class TactileTestEnv(MujocoEnv):
    ...
    def _load_model(self):
        ...
        """
        super()._load_model()
        size_x, size_y, size_z = 0.01289/2, 0.0128/2, 0.01/2  # half of length is the definition of size for box
        density = (self.weight+2.1)/1000/((size_x* size_y* size_z)*(2**3))  # weight of the base is 2.1 grams
        print("Cube Density: {}".format(density))

        cube = BoxObject(
            size=(size_x, size_y, size_z),
            friction=1.,
            density=density,  # 61881.788 kg/(m^3), calculated with: (100+2.1)/1000 kg = 0.01289 * 0.0128 * height * density
            rgba=[0.8, 0, 0, 0.9],
        )
...
if __name__ == '__main__':
    ...
    def print_contact_info(env):
        for i in range(env.sim.data.ncon):  # total number of contact: env.sim.data.ncon
            ...
            print('Contact {}: {} and {}'.format(i, env.sim.model.geom_id2name(c.geom1), env.sim.model.geom_id2name(c.geom2)))
    ...
    for w in WEIGHTS:
        for d in DISTANCE:
            ...
            env.sim.model.opt.gravity[:] = [0,0,-9.81]  # This will change the gravity in simulation. Ref: https://github.com/openai/mujoco-py/issues/459
